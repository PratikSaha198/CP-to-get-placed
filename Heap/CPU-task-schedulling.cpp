...
{
    // Adding the index too, for the final answer
    for(int i=0;i<v.size();i++){
        v[i].push_back(i);
    }

    // Gets sorted according to starting time
    sort(v.begin(), v.end());

    // The tasks being available the first are needed firstly, so min heap is used.
    ...
    while(i<n || pq.size())
    {
        ...
        // If the curr time is more than the time when one task becomes avaiable then push into min heap and increase index 
        ...
        // The one with the minimum processing time is popped and that amount of time is added to 'time'
